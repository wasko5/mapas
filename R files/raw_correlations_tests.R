...
data <- read_excel(paste(INPUT_BASE_DIR, "raw_correlations_tests_withMissingData.xlsx", sep=""))
...
for (row in 1:length(rownames(corr_matrix))) {
  ...
  curr_n = min(sum(!is.na(data[corr_matrix[row, "Variable1"]])),
               sum(!is.na(data[corr_matrix[row, "Variable2"]])))
...
corr_matrix <- corr_matrix[, c("Variable1", "Variable2", "Correlation_Coefficient",
                               "CI_low", "CI_high", "pvalues")]

write_xlsx(corr_matrix, paste(OUTPUT_BASE_DIR, "raw_correlations_tests_spearman_withMissingData.xlsx", sep=""))
