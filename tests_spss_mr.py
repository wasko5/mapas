...
def spss_mr_tests_minimalStats():
	...
	#setup - actual
	input_df = pd.read_excel(os.path.join(global_vars.INPUT_DATAFRAMES_FOLDER, "spss_mr_tests_minimalStats.xlsx"))
	...
	expected_df_r = pd.read_excel(os.path.join(global_vars.OUTPUT_DATAFRAMES_FOLDER, "spss_mr_tests_minimalStats.xlsx"), dtype = object)
...
def spss_mr_tests_allStats():
	...
	#setup - actual
	input_df = pd.read_excel(os.path.join(global_vars.INPUT_DATAFRAMES_FOLDER, "spss_mr_tests_allStats.xlsx"))
	mod_input_df = decision_funcs.modify_raw_data_df(input_df)
	actual_df = decision_funcs.generate_output_df(mod_input_df)

	#setup - expected
	#forcing dtype to object as data formatting is done within the output_df func
	expected_df_r = pd.read_excel(os.path.join(global_vars.OUTPUT_DATAFRAMES_FOLDER, "spss_mr_tests_allStats.xlsx"), dtype = object)
	#forcing the interpreted nan value to be empty as in the actual_df
	expected_df_r.iloc[0, 3] = actual_df.iloc[0, 3]
